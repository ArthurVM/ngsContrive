...
def parseMixFile(mix_file):
    """ Parse the mix file
    """
    readnum_dict = defaultdict(float)
    with open(mix_file, "r") as fin:
        for line in fin.readlines():
            id, readnum = line.split(" ")
            readnum_dict[id] = int(readnum)

    sumreads = sum([readnum for id, readnum in readnum_dict.items()])
    print(f"Simulating {sumreads} reads...")
    return readnum_dict
...
def genDataset(prop_dict, input_dir, out, seed):
    forward_reads = []
    reverse_reads = []

    for id, p in prop_dict.items():
        numreads = p
        print(f"Getting {numreads} reads from {id}", file=sys.stdout)
...
def main(argv):
    ...
    genDataset(prop_dict, args.input_dir, args.out_prefix, args.seed)
